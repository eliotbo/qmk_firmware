 ...
 *
 * Layers:
 * - 0: BASE (Default: White LEDs, host-controllable in all layers)
 * - 1: BUY (Default: Green LEDs, host-controllable)
 * - 2: SELL (Default: Red LEDs, host-controllable)
 *
    ...
    switch (data[0]) {
        ...
        case CMD_SET_MODE:
            if (length >= 2) {
                ...
                if (mode <= 2) {
                    layer_move(mode);
                    // Don't clear HID override - host controls LEDs in all layers
                }
            }
            break;
...
layer_state_t layer_state_set_user(layer_state_t state) {
    ...
    // Don't clear HID override when changing layers - host controls LEDs
    ...
    switch (current_layer) {
        case 0:
            ...
            break;
        case 1:
            // Green for buy layer (unless overridden by HID)
            if (!hid_rgb_override) {
                rgb_matrix_set_color_all(RGB_GREEN);
            }
            break;
        case 2:
            // Red for sell layer (unless overridden by HID)
            if (!hid_rgb_override) {
...
void rgb_matrix_indicators_user(void) {
    // Apply HID override regardless of layer
    if (hid_rgb_override) {
    ...
    // Apply layer colors when NOT in HID override mode
    switch (current_layer) {
        case 0:
            rgb_matrix_set_color_all(RGB_WHITE);
            break;
